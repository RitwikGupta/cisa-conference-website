...
Design tokens live at the start of `src/styles/global.css`. The full-width SVG aperture background is in `src/components/Aperture.astro`; no raster or video is needed. Keep it behind the homepage introduction on warm white, with a feathered overlay protecting the text. On mobile it stays a faint background, without a separate illustration block or added hero height. Keep the terrapin and sampling arc fully inside the hero. Preserve uniform SVG scaling with `preserveAspectRatio="xMaxYMid meet"`; the mobile SVG uses the same aspect ratio as its viewBox, a positive right inset, and space above the action links. Do not stretch its width and height independently or use negative offsets that clip the subject. Keep the pause control clear of the buttons and allow pointer events only on the control. The 12-second sequence collects measurements over seconds 0–8, holds the resolved terrapin over seconds 8–10, and crossfades back to the initial estimate over seconds 10–12. The sampling arc sits on the right. One camera moves continuously along the arc, leaving nine faint emission marks. Dark outgoing pulses originate at the camera’s emission position; brighter echoes follow separate paths that meet its new position at reception. The reception ring moves with the camera. Build-time geometry generates the camera’s CSS keyframes and the emission/return ray endpoints from the same trajectory, including exact emission and reception times. Keep these synchronized when changing the timing. The camera returns smoothly along the arc during seconds 8–12 while the reconstruction holds and resets. Complete all round trips before second 8, when the resolved image holds. Outgoing and returning streaks use a dash gap longer than the normalized path so no duplicate pulse appears at the opposite end. An original top-view terrapin drawing refers to the University of Maryland: a diffuse estimate becomes a soft silhouette, then a sharp outline with shell details. The intermediate silhouette uses a small, fixed SVG blur; only opacity is animated between reconstruction states. This is a schematic, not a quantitative reconstruction simulation or an official university mark. Keep the strongest contrast outside the copy area. Retain the resolved static image for reduced motion and JavaScript-disabled visits, the accessible pause/resume control, and automatic pauses when offscreen or in a hidden tab. Automatic pauses must not override a visitor’s manual pause.
